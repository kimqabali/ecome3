...
@section('title', \App\CPU\translate('add_Neighborhood'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{\App\CPU\translate('add_Neighborhood')}}</li>
            </ol>
        </nav>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{\App\CPU\translate('add_Neighborhood')}}
                    </div>
                        ...
                        <form action="{{route('admin.Addgovernorate')}}" method="POST">
                            ...
                            <div class="row">
                                    ...
                                    @foreach(json_decode($language) as $lang)
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <input type="text" name="name[]" class="form-control"
                                                   placeholder="  {{ \App\CPU\translate('Neighborhood')}}" {{$lang == $default_lang? 'required':''}}>
                                        </div>
                                ...
                                <div class="col-12 col-md-5 form-group">
                                    ...
                                    {{-- <input type="text" name="city_advertis_id" class="form-control"> --}}
                                    <select name="city_advertis_id" class="form-control">
                                        @foreach ($CityAdvertis as $CityAdvertises)
                                            <option value="{{ $CityAdvertises->id }}">{{ $CityAdvertises->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-bordered" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th>{{ \App\CPU\translate('Neighborhood')}}</th>
                                ...
                                <tbody>
                                    @foreach ($Governorate as $Governorates)
                                        <tr>
                                            <td>{{ $Governorates->id }}</td>
                                            <td>{{ $Governorates->name }}</td>
                                            <td>{{ $Governorates->CityAdvertis->name }}</td>
                                            <td class="d-flex">
                                                <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                    title="{{ \App\CPU\translate('Edit')}}"
                                                   href="{{route('admin.Editgovernorate',[$Governorates['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.deletegovernorate')}}",
                        method: 'delete',
                        data: {id: id},
                        success: function () {
