            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                        <form action="{{route('admin.updategovernorate', $governorate->id)}}" method="POST">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-12 col-md-5">
                                    @foreach(json_decode($language) as $lang)
                                        <?php
                                        if (count($governorate['translations'])) {
                                            $translate = [];
                                            foreach ($governorate['translations'] as $t) {
                                        ...
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <input type="text" name="name[]"
                                                   value="{{$lang==$default_lang?$governorate['name']:($translate[$lang]['name']??'')}}"
                                                   class="form-control"
                                ...
                                <div class="col-12 col-md-5 form-group">
                                    ...
                                    <select name="city_advertis_id" class="form-control">
                                        @foreach ($CityAdvertis as $CityAdvertises)
                                            <option value="{{ $CityAdvertises->id }}" @if ($CityAdvertises->id == $governorate->city_advertis_id)
                                                selected
                                            @endif>{{ $CityAdvertises->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
