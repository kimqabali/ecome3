...
@section('title', \App\CPU\translate('add_extra_benefit'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{\App\CPU\translate('add_extra_benefit')}}</li>
            </ol>
        </nav>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('add_extra_benefit')}}
                    </div>
                    <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                        <form action="{{route('admin.AddExtraBenefit')}}" method="POST">
                            @csrf
                                    ...
                                    @foreach(json_decode($language) as $lang)
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <input type="text" name="name[]" class="form-control"
                                                   placeholder="  {{ \App\CPU\translate('add_extra_benefit')}}" {{$lang == $default_lang? 'required':''}}>
                                        </div>
                            ...
                            <table id="dataTable" class="table table-bordered" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($extraBenefit as $extraBenefits)
                                        <tr>
                                            <td>{{ $extraBenefits->id }}</td>
                                            <td>{{ $extraBenefits->name }}</td>
                                            <td class="d-flex">
                                                <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                    title="{{ \App\CPU\translate('Edit')}}"
                                                   href="{{route('admin.EditExtraBenefit',[$extraBenefits['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.deleteExtraBenefit')}}",
                        method: 'delete',
                        data: {id: id},
                        success: function () {
