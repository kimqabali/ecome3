...
@section('title', \App\CPU\translate('add_governorate'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{\App\CPU\translate('add_governorate')}}</li>
            </ol>
        </nav>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('add_governorate')}}
                    </div>
                    <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                        <form action="{{route('admin.AddCityAdvertis')}}" method="POST">
                            @csrf
                                    ...
                                    @foreach(json_decode($language) as $lang)
                                        <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            ...
                                            <input type="text" name="name[]" class="form-control"
                                                   placeholder="   {{ \App\CPU\translate('governorate')}}" {{$lang == $default_lang? 'required':''}}>
                                        </div>
                            ...
                            <table id="dataTable" class="table table-bordered" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($CityAdvertis as $CityAdvertises)
                                        <tr>
                                            <td>{{ $CityAdvertises->id }}</td>
                                            <td>{{ $CityAdvertises->name }}</td>
                                            <td>{{ $CityAdvertises->StateAdvertis->name }}</td>
                                            <td class="d-flex">
                                                <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                    title="{{ \App\CPU\translate('Edit')}}"
                                                   href="{{route('admin.EditCityAdvertis',[$CityAdvertises['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                                <a class="btn btn-danger btn-sm delete" style="cursor: pointer;"
                                                    title="{{ \App\CPU\translate('Delete')}}"
                                                   id="{{$CityAdvertises['id']}}">
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.deleteCityAdvertis')}}",
                        method: 'delete',
                        data: {id: id},
                        success: function () {
